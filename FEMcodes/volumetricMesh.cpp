...
// saves the mesh to a .veg file
int VolumetricMesh::saveToAscii(const char * filename, elementType elementType_) const
{
...
// saves the mesh to a FE .obj file
int VolumetricMesh::saveToFE_obj(const char * filename, elementType elementType_) const
{
  FILE * fout = fopen(filename, "w");
  if (!fout)
  { printf("Error: could not write to %s.\n",filename);return 1;}

  fprintf(fout, "TITLE = \" 3 Dimensional Tetrahedral FE mesh data \" \n");
  fprintf(fout, "VARIABLES = \"X\", \"Y\", \"Z\" \n");
  fprintf(fout, "ZONE NODES=%d, ELEMENTS=%d, DATAPACKING=POINT, ZONETYPE=FETETRAHEDRON \n",numVertices,numElements);
...
int VolumetricMesh::saveDeformationToFE_obj(const char * filename, double *u_, double *f_int_, elementType elementType_) const
{
  double deform_,internalForce_; FILE * fout = fopen(filename, "w");
  if (!fout)
  {printf("Error: could not write to %s.\n",filename);return 1;}

  fprintf(fout, "TITLE = \" 3 Dimensional Tetrahedral FE mesh data \" \n");
  fprintf(fout, "VARIABLES = \"X\", \"Y\", \"Z\", \"Deformation\", \"f_internal\" \n");
  fprintf(fout, "ZONE NODES=%d, ELEMENTS=%d, DATAPACKING=POINT, ZONETYPE=FETETRAHEDRON \n",numVertices,numElements);
  ...
  {
    Vec3d v = *getVertex(i);
    deform_=sqrt(pow(u_[0+i*3],2.0)+pow(u_[1+i*3],2.0)+pow(u_[2+i*3],2.0));
    internalForce_=sqrt(pow(f_int_[0+i*3],2.0)+pow(f_int_[1+i*3],2.0)+pow(f_int_[2+i*3],2.0));
    fprintf(fout,"%.15G %.15G %.15G %.15G %.15G \n",v[0]+u_[0+i*3],v[1]+u_[1+i*3],v[2]+u_[2+i*3],deform_,internalForce_);
  }
  // write elements
  for(int el=0; el < numElements; el++)
  {
